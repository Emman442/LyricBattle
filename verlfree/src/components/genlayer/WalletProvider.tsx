...
import { error, userRejected } from "@/components/utils/toast";
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      const explicitlyDisconnected = localStorage.getItem(DISCONNECT_FLAG) === "true";
  ...
  useEffect(() => {
    ...
    const handleAccountsChanged = async (accounts: string[]) => {
      ...
      // If accounts changed and we have one, user is actively using wallet → clear disconnect flag
